...
In production APIs, different clients (free, pro, enterprise) get different rate limits based on their API key tier.
...
```javascript
const RATE_LIMITS = {
  free:       { windowMs: 60000, maxRequests: 30 },    // 30 req/min
  pro:        { windowMs: 60000, maxRequests: 300 },   // 300 req/min
  enterprise: { windowMs: 60000, maxRequests: 3000 }   // 3000 req/min
};
```

## Rate Limiter Middleware with Redis Sliding Window
...
async function tieredRateLimiter(req, res, next) {
  ...
  pipeline.zremrangebyscore(redisKey, 0, now - windowMs); // Remove expired entries
...
# HMAC Request Signing for Webhook / API Security

HMAC (Hash-based Message Authentication Code) signing ensures that:
1. The request was sent by someone who knows the shared secret.
2. The payload has not been tampered with in transit.

## How It Works

1. The sender computes a HMAC-SHA256 hash of the request body using a shared secret.
2. The hash is sent in a header (e.g., `X-Signature`).
3. The receiver computes the same hash and compares — if they match, the request is authentic.

## Signing Requests (Sender Side)
...
async function sendSignedRequest(url, data, secret) {
  ...
  return axios.post(url, data, {
    headers: {
      ...
      'X-Timestamp': Date.now().toString() // Prevent replay attacks
    }
  });
}
```

## Verifying Signed Requests (Receiver Side)

```javascript
function verifyHMACSignature(secret, maxAgeMs = 300000) {
  return (req, res, next) => {
    ...
    if (Date.now() - timestamp > maxAgeMs) {
      return res.status(401).json({ error: 'Request too old — possible replay attack' });
    }
...
router.post(
  '/webhooks/partner',
  express.json(),
  verifyHMACSignature(process.env.PARTNER_WEBHOOK_SECRET),
  (req, res) => {
...
- Always use **`crypto.timingSafeEqual()`** instead of `===` to compare signatures — prevents timing attacks.
- Include a **timestamp** and reject old requests to prevent replay attacks.
- Use **HMAC-SHA256** as the minimum — never MD5 or SHA1.
- The **shared secret** should be long (32+ chars), random, and stored securely (environment variables, secrets manager).
